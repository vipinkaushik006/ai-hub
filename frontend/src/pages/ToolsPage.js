...
const CATEGORIES = ['All', 'AI Writing', 'AI Coding', 'AI Image Generation', 'AI Video', 'AI Audio', 'AI Data', 'Productivity'];
...
export default function ToolsPage() {
  ...
  useEffect(() => {
    if (user) {
      api.get('/users/bookmarks').then(res => setBookmarks(res.data.map(b => b._id))).catch(() => {});
    }
  }, [user]);

  const handleBookmark = async (toolId) => {
    if (!user) { toast.error('Please login to bookmark tools'); return; }
    try {
  ...
  const handleSearchSubmit = (e) => { e.preventDefault(); setParam('search', localSearch); };

  return (
    <>
      <SEO title="AI Tools Directory" description="Browse and search 200+ AI tools across writing, coding, image generation, and video categories." keywords="AI tools directory, best AI tools, ChatGPT alternatives, AI writing tools, AI coding tools" />

      <div className="pt-24 pb-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          {/* Header */}
          <div className="text-center mb-12">
            <div className="text-primary-400 font-semibold text-sm uppercase tracking-wider mb-2">🤖 AI Tools Directory</div>
            ...
            <p className="text-slate-400 text-lg max-w-2xl mx-auto">Find the perfect AI tool for writing, coding, image generation, video, and more.</p>
          </div>
          ...
          <div className="card p-5 mb-8">
            ...
            <div className="flex flex-wrap gap-3">
              <div>
                <p className="text-xs text-slate-500 mb-2 uppercase tracking-wider">Category</p>
              ...
              <div>
                <p className="text-xs text-slate-500 mb-2 uppercase tracking-wider mt-3">Pricing</p>
                <div className="flex flex-wrap gap-2">
          ...
          {loading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {[...Array(12)].map((_, i) => <div key={i} className="card p-5 h-52 skeleton" />)}
            </div>
          ) : tools.length === 0 ? (
            <div className="text-center py-20">
              <div className="text-6xl mb-4">🔍</div>
              <h3 className="font-display font-bold text-white text-2xl mb-2">No tools found</h3>
              <p className="text-slate-400">Try adjusting your search or filters</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {tools.map(tool => (
                <ToolCard key={tool._id || tool.slug} tool={tool} onBookmark={handleBookmark} isBookmarked={bookmarks.includes(tool._id)} />
              ))}
            </div>
          )}
